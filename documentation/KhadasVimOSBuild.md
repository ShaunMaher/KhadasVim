...
## Caveats
  ...
  * At the time of writing, I have been unable to get any Device Tree file
    ...
    this issue will solve my first issue.  I think this might actually be a
    u-boot issue.
  * The mainline u-boot has some issues that the Khadas u-boot fork does not
    (see "Build u-boot (mainline)" below).  The above issue with Device Trees
    *I think* stops me using the mainline kernel so I'm kind of stuck with
    Khadas u-boot and Khadas 4.9 kernel at the moment.  More work is required in
    this regard.
...
mkdir u-boot
cd u-boot
wget https://github.com/u-boot/u-boot/archive/7d22c8e.tar.gz
...
patches:
```
wget https://cdn.kernel.org/pub/linux/kernel/v4.x/linux-4.14.11.tar.xz
tar --strip-components=1 -xJf linux-4.14.11.tar.xz
git apply $(find ../patches/linux/mainline-4.14.11/ -mindepth 1 | sort)
```
